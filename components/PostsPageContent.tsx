...
export function PostsPageContent() {
  ...
  const { data: postsData, isLoading: postsLoading } = useSWR<PostsResponse>(
    '/api/posts?where[status][equals]=published&sort=-publishedAt&limit=100',
    fetcher
  )

  const { data: categoriesData, isLoading: categoriesLoading } =
    useSWR<CategoriesResponse>('/api/categories?limit=100', fetcher)

  const posts = formatPosts(postsData)
